...
import { EventDoc, Event } from '@/models/event.model';
import { fromDoc } from '@/models/util';
import { ObjectId } from 'mongodb';

export const createEvent = async (data: Partial<EventDoc>, user: any): Promise<Event> => {
    const col = await events();
    const doc: EventDoc = {
        ...(data as EventDoc),
        price: data.price ?? 0,
        imgUrls: data.imgUrls ?? [],
        status: "DRAFT", // Rule: Initial status is always DRAFT
    ...
    const converted = fromDoc(created);
    if (!converted) throw new Error("Failed to create event");
    
    // Convert organizerId from ObjectId to string for Event type
    return {
        ...converted,
        organizerId: (created?.organizerId as ObjectId).toString(),
    } as Event;
};

export const updateEvent = async (id: string, update: Partial<EventDoc>) => {
    const col = await events();
...
export const deleteEvent = async (id: string) => {
    const regCount = await (await registrations()).countDocuments({ 
        eventId: new ObjectId(id), 
        status: "CONFIRMED" 
    });
    
    if (regCount > 0) throw new Error("CONFLICT_HAS_REGISTRATIONS");
    
    await (await events()).deleteOne({ _id: new ObjectId(id) });
};

export const publishEvent = async (id: string, userId: string) => {
    const col = await events();
    const event = await col.findOne({ _id: new ObjectId(id) });
    if (!event) throw new Error("NOT_FOUND");

    // Logic: First published event -> PENDING_REVIEW, others -> APPROVED
    const approvedCount = await col.countDocuments({ 
        organizerId: new ObjectId(userId), 
        status: "APPROVED" 
    });
    
    const newStatus = approvedCount === 0 ? "PENDING" : "APPROVED";
...
export const getMyEvents = async (userId: string) => {
    const col = await events();
    const pipeline = [
        { $match: { organizerId: new ObjectId(userId) } },
        {
            $lookup: {
                from: "Registration",
                localField: "_id",
    ...
    return docs.map(doc => fromDoc(doc)).filter(d => d !== null);
};

export const getEventById = async (id: string): Promise<Event | null> => {
    const col = await events();
    const doc = await col.findOne({ _id: new ObjectId(id) });
    return fromDoc(doc) as Event | null;
};