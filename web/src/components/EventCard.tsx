...
export function EventCard({ event, onClick }: EventCardProps) {
  ...
  const getStatusBadge = () => {
    const now = Date.now();
    const diff = eventTime - now;
    if (isPast) return <span className="badge bg-secondary px-3 py-2 opacity-75">Past Event</span>;
    ...
    return (
      <span className="badge bg-primary px-3 py-2 shadow-sm">
        {new Date(event.eventDateTime).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}
      </span>
    );
  };
  ...
  return (
    <motion.div
      ...
      style={{ 
        ...
        cursor: "pointer",
        // FIX: Using a solid, deep background so light-mode white background doesn't bleed through
        background: "#1e293b", 
    ...
    >
      ...
      {/* FIX: Explicitly using text-white to override Light Mode defaults */}
      <div className="card-body p-4 text-white">
        <div className="d-flex justify-content-between align-items-start mb-2">
          <h5 className={`fw-bold mb-0 text-truncate text-white ${isPast ? 'opacity-50' : ''}`} style={{ maxWidth: '70%' }}>
            {event.title}
          </h5>
        ...
        <div className="d-flex align-items-center gap-2 mb-2">
            {/* FIX: Changed text-secondary to text-white-50 for consistent visibility */}
            <p className="text-white-50 small mb-0">📍 {event.location}</p>
            {event.avgRating && (
                <span className="ms-auto small fw-bold text-warning">⭐ {event.avgRating}</span>
            )}
        </div>

        {/* FIX: Changed text-secondary to text-white-50 */}
        ...
        <div className="d-flex flex-wrap gap-1 mb-3">
            {event.tags?.split(',').slice(0, 3).map((tag, i) => (
                <span key={i} className="badge bg-white bg-opacity-10 text-white border border-white border-opacity-25 small fw-normal">
                    {tag.trim()}
                </span>
            ))}
        </div>
        ...
        <div className="pt-3 border-top border-white border-opacity-10 d-flex justify-content-between align-items-center">
            {/* FIX: Changed text-info to a brighter hex for dark backgrounds */}
            <small className={isPast ? "text-white-50" : ""} style={{ color: isPast ? "" : "#7dd3fc" }}>
